...
@profile
def blast_search(event_id: str, blast : str, db : str, fasta : Bio.File._IndexedSeqFileDict, chrom : str, const_region_start : int, const_region_end : int, strand : str, gene : str):
    import subprocess
    import re
    ...
    blastFile = open('small_test_input_search.fasta', 'w')
...
def exon_skip(junctions : pandas.DataFrame, spladderOut : str, outdir : str) -> None:
    ...
    *** needs testing ***
    ...
    if args.frameMatch:
        ...
        unique_events_of_interest = unique_events_of_interest[:20]

        unique_events_of_interest[['upstream_region_translated_frame', 'annotated_peptide', 'blast_expected_value', 'blast_gene']] = ''
        for idx, row in unique_events_of_interest.iterrows():
            print(idx)
            if row['strand'] == '+':
                blast_results = blast_search(event_id = row['event_id'], blast = args.blast, db = args.blastDb, fasta = dna_fasta, chrom = row['chrom'], const_region_start = int(row['exon_pre_start']), const_region_end = int(row['exon_pre_end']), strand = row['strand'], gene = row['symbol'])
                ...
                blast_results = blast_search(event_id = row['event_id'], blast = args.blast, db = args.blastDb, fasta = dna_fasta, chrom = row['chrom'], const_region_start = int(row['exon_aft_start']), const_region_end = int(row['exon_aft_end']), strand = row['strand'], gene = row['symbol'])
    ...
    '''  
    unique_events_of_interest.to_csv("/home/tonya/software/neoantigENcyclopedia/neojunction/small_test_exon_skip_testing_blast_03212021.txt", sep = '\t', index = False)
    '''
    unique_events_of_interest = pandas.read_csv("/home/tonya/software/neoantigENcyclopedia/neojunction/small_test_exon_skip_testing_blast_03212021.txt", sep = '\t')
